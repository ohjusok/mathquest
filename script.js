...
function adventurizeJuneDeepStage(stage, day, week, index) {
  const questNames = [
    [
      "새벽 나루터의 배달 의뢰",
      "마을 정원의 숨은 입구",
  ...
  const openers = [
    [
      `토리는 ${day}일째 아침, 안개가 낮게 깔린 나루터에서 첫 의뢰를 받았습니다. 약초 상자를 안전하게 옮겨야 해서 빠른 길보다 흔들림이 적은 길을 골라야 하고, 돌아올 때는 짐이 가벼워져 다른 길을 선택할 수 있습니다.`,
      `토리는 마을 사람들이 쉬어 갈 정원을 고치는 일을 맡았습니다. 정원 모양은 단순하지만, 사람들이 드나드는 문 자리는 비워 두어야 하므로 전체 둘레를 그대로 주문하면 재료가 남습니다.`,
      `토리는 다음 마을로 가는 기차표를 놓치면 오늘의 모험을 이어 갈 수 없습니다. 걷는 시간, 표를 사는 시간, 미리 승강장에 서 있을 시간을 모두 한 계획 안에 넣어야 합니다.`,
      `토리는 사원으로 이어지는 길에서 숫자가 적힌 등불을 발견했습니다. 숫자는 일정한 수를 계속 더하는 것이 아니라, 더해지는 수 자체가 조금씩 달라지는 방식으로 놓여 있습니다.`,
      `토리가 오래된 성문 앞에 서자 숫자 암호판이 나타났습니다. 암호는 두 자리 수이고, 안내판의 두 조건을 모두 만족해야 문이 열립니다. 한 조건만 맞는 보기는 함정입니다.`
    ],
    [
      `토리는 항구 축제 준비를 도우며 긴 리본을 나누어 써야 합니다. 무대와 입구 장식에 쓰는 리본은 서로 겹치지 않으므로, 전체에서 두 부분을 각각 찾아 빼야 합니다.`,
      `마법 온실 관리인은 토리에게 약초 바구니와 기록표를 건넸습니다. 바구니에는 kg과 g이 함께 적혀 있지만, 기록표에는 남은 양을 g으로만 써야 합니다.`,
      `토리는 궁전 정원 바닥을 설계하고 있습니다. 정원 전체는 직사각형이지만 가운데 연못 자리는 타일을 깔 수 없어서, 전체 넓이와 비워 둘 넓이를 구분해야 합니다.`,
      `토리는 용사 훈련장에서 세 번의 훈련 기록을 확인합니다. 평균 점수는 적혀 있지만 마지막 점수가 번져 보이지 않아, 평균을 전체 점수로 바꾸어 거꾸로 찾아야 합니다.`,
      `토리는 떠돌이 시장에서 여행 도구를 여러 개 골랐습니다. 계산대에서는 쿠폰을 물건마다 빼 주는 것이 아니라, 전체 금액에서 딱 한 번만 빼 줍니다.`
    ],
    [
      `토리는 늪지대를 건너기 위해 비율이 정해진 빛나는 물약을 만들어야 합니다. 물약 책에는 두 재료의 비가 적혀 있고, 오늘은 그 비율 묶음을 여러 번 반복해야 합니다.`,
      `토리는 밤바다를 건너기 전 두 등대가 동시에 켜지는 순간을 기다립니다. 두 등대는 서로 다른 간격으로 켜지므로, 두 간격이 처음으로 다시 만나는 시간을 찾아야 합니다.`,
      `토리는 수정 광산에서 찾은 조각들을 상자에 담으려고 합니다. 상자의 바닥 넓이만으로는 충분하지 않고, 높이까지 포함한 공간의 크기를 생각해야 합니다.`,
      `토리는 강가 지도에 자신의 이동 기록을 남깁니다. 일정한 속도로 걸은 구간과, 속도를 재지 못했지만 길이가 알려진 징검다리 구간을 함께 계산해야 합니다.`,
      `토리는 어두운 동굴에서 색이 다른 보석이 섞인 주머니를 받았습니다. 눈으로 확인하지 않고 하나만 뽑으므로, 원하는 경우의 수와 전체 경우의 수를 비교해야 합니다.`
    ],
    [
      `토리는 마지막 성으로 가기 전 축제 상점에서 모험 도구를 삽니다. 상점 주인은 먼저 퍼센트 할인을 해 주고, 그 다음 계산대에서 종이 쿠폰을 한 번 더 받아 줍니다.`,
      `수정문은 토리가 말한 수를 그대로 보여 주지 않고, 정해진 계산을 한 뒤 결과만 벽에 비춥니다. 토리는 결과에서 출발해 처음 수를 되돌려 찾아야 합니다.`,
      `토리는 별빛 지도에서 다음 숫자를 찾지 못하면 길을 잃습니다. 숫자 자체만 보지 말고, 이웃한 숫자 사이의 차이가 어떻게 변하는지 한 번 더 살펴야 합니다.`,
      `궁전 정원에는 큰 정사각형 바닥 안에 작은 정사각형 연못이 있습니다. 토리가 걸을 수 있는 곳은 연못이 아니라 그 둘레의 바닥이므로, 두 넓이의 차를 구해야 합니다.`,
      `토리는 마지막 섬으로 가는 배에 상자를 싣습니다. 작은 상자와 큰 상자의 무게가 다르므로, 종류별 개수와 무게를 따로 계산한 뒤 합쳐야 합니다.`
    ]
  ];
  const slot = index - 5;
  return {
    ...stage,
    title: questNames[week - 1][slot],
    story: "",
    problem: [
      `${openers[week - 1][slot]} ${stage.problem[0]}`,
      ...stage.problem.slice(1)
    ]
  };
}
...
function makeJuneQuest(day) {
  ...
  const stages = Array.from({ length: 10 }, (_, index) => {
    if (index < 5) return makers[week - 1](day, index + 1);
    return adventurizeJuneDeepStage(makeJuneDeepStage(day, index + 1, week), day, week, index);
  });
  return {
  ...
  return `
    <article class="slide stage-question" aria-label="스테이지 ${index + 1} 문제">
      ...
      <section class="question-card">
        ...
        <h2>${stage.title}</h2>
        ${stage.story ? `<p class="story">${stage.story}</p>` : ""}
